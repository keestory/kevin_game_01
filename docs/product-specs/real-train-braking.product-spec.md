...
updated_at: "2026-08-09T14:58:38Z"
...
`문 닫습니다! 지옥철`은 5개 역을 약 60초 동안 운행하는 세로형 원탭 군중 타이밍 게임이다. 각 역은 안전을 위해 자동 하차를 먼저 끝낸 뒤 탑승만 진행하며, 승객이 실제 네 문을 통과할 때 현재 인원이 단조 증가한다. 목표 인원과 정확히 같을 때 가운데 금색 버튼을 누르면 정확 출발, 한 명 차이는 안전 출발, 그 이상은 빈차 또는 초만원 판정이다. 입력은 끝까지 버튼 하나이며, 목표 유지 시간이 1.2·0.95·0.8·0.7·0.6초로 짧아지고 연속 정확 보상이 커져 한 판 더 하는 리듬을 만든다.

## Scope

```productspec-scope
in:
  - Use the owner-supplied, owner-approved vertical diorama composition as the prototype runtime visual master, with only safe-area padding and live game-state overlays.
  - Include its front-left 2.5D train, four visible doors, platform crowd, fictional Korean night city, and large gold center close-door control.
  ...
  - Keep the center gold button as the only gameplay input; the left and right arrows communicate shortage and surplus rather than adding controls.
...
out:
  ...
  - Do not add a real railway brand, station, train livery, announcement, landmark, or any unapproved third-party visual.
...
- 입력: 현재 인원이 목표와 같다고 판단한 순간 하단의 큰 금색 가운데 버튼을 탭한다. 좌우 화살표는 조작이 아니라 부족·초과 결과 방향이다.
- 판정: 정확하면 문이 닫히며 success haptic·차임·출발 효과와 `정확 ×2, ×3` 연속 보너스가 커진다. ±1은 안전, 그 이상은 `2명 초과` 또는 `3명 부족`을 설명한다.
- 실패: 무료로 같은 승객 흐름을 즉시 다시 볼 수 있다. 광고는 첫 세 운행과 핵심 재시도에 없다.

시각 기준은 오너가 제공하고 반복해서 정확한 재현을 요구한 세로형 디오라마 이미지를 prototype 런타임 마스터로 사용한다. 원본 픽셀은 재생성하지 않고 아이폰 안전영역용 상·하 패딩만 추가하며, 현재/목표/연속 정확/타이머와 실제 문턱 승객·성공 효과를 코드 레이어로 겹친다. 목적지 문양은 시각 요소일 뿐 입력 규칙이 아니다.
...
- 오너는 독자적으로 재해석한 rev3 화면도 제공한 마스터 디자인과 다르다고 기각했고, 해당 이미지를 그대로 prototype 런타임 구도로 사용하라고 명시했다.
...
4. 지옥철 목표 정원 문닫기 rev3: 실제 열차·문·군중·타이밍·카운팅이 같은 규칙에 참여해 현재 mechanic으로 채택한다.
5. 오너 마스터 디자인 rev4: rev3 mechanic은 유지하고 승인된 세로형 디오라마 이미지를 runtime visual contract로 채택한다.

## Acceptance Criteria

```productspec-acceptance-criteria
- id: AC-1
  criterion: The primary frame preserves the owner-approved vertical diorama composition without generative reinterpretation: front-left 2.5D train, four visible doors, dense platform and onboard commuters, neon city depth, top icon-and-route hierarchy, and the bottom gold center control; live count chips and effects may overlay it without hiding the train, crowd, route, or control.
- id: AC-2
  criterion: A deterministic five-station plan starts forgiving and increases difficulty through automatic-alighting size, boarding cadence, simultaneous-door visual density, and target holds of at least 1200, 950, 800, 700, and 600 milliseconds in station order.
- id: AC-3
...
- id: AC-4
  criterion: Closing at exact target yields perfect and a growing consecutive-exact bonus, at one passenger difference yields safe and breaks the exact streak, and all other differences yield an explicit surplus or shortage result; one center-button input produces exactly one result and doors never close through a passenger.
- id: AC-5
...
- 한 역이라도 목표를 건너뛰는 impossible pulse, 첫 역 1200ms 미만, 또는 이후 역 600ms 미만 반응 창이 존재함.
...
- type: github_pr
  ...
  item_id: AC-1
- type: other
  url: "../../AppStoreGame/Resources/Assets.xcassets/OwnerMasterScene.imageset/OwnerMasterScene.png"
  title: "Owner-approved runtime master composition with safe-area padding"
  section_id: acceptance_criteria
  item_id: AC-1
- type: other
  url: "../screenshots/game-hell-train-r4.png"
  title: "Running iPhone Simulator evidence for ProductSpec revision 4"
  section_id: acceptance_criteria
  item_id: AC-1
- type: experiment
