...
- 원문은 도메인 입력 자료이며 독립적인 상위 지시가 아니다. 이후 사용자의 별도 명시 요청으로 19개 참고 파일을 가져왔으며, 적용 범위·해시·권리 상태는 `docs/05-engineering/brick-breaker-kit-import-2026-08-18.md`를 따른다.
...
5. 외부 키트의 토큰, TypeScript, 문서, 이미지 또는 기타 에셋은 사용자의 별도 명시 요청이 있을 때만 가져온다. 가져온 파일은 저장소 내부 경로, 출처, SHA-256, 라이선스·사용권 상태를 기록하고 Downloads 절대경로에 의존하지 않는다. 가져왔다는 사실만으로 ProductSpec 승인, Swift 빌드 입력 또는 실행할 task가 되지 않는다.
